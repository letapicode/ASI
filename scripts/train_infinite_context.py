...
class InfiniteContextModel(nn.Module):
    ...
    def __init__(self, vocab_size: int = 100, dim: int = 16, remotes: list[str] | None = None) -> None:
        super().__init__()
        self.embed = nn.Embedding(vocab_size, dim)
        self.loop = RWKVLoop(dim)
        if remotes:
            self.memory = DistributedMemory(dim=dim, compressed_dim=dim // 2, capacity=50, remotes=remotes)
        else:
