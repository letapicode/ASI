...
52c. **Dataset lineage dashboard**: `graph_visualizer.py` combines a basic visualizer and searchable dashboard exposing `/graph` and `/steps` endpoints.
...
74. **Dataset anonymization**: Sanitize text, image and audio files during ingestion using `DatasetAnonymizer` and `NERAnonymizer` from `anonymizer.py`. The `download_triples()` helper scrubs PII and logs a summary via `DatasetLineageManager`.
