...
- `src/moe_router.py` implements a hash-based router with `load_balance_std` for expert utilization.
- `src/flash_attention3.py` wraps the FlashAttention‑3 kernel and falls back to PyTorch when missing.
- `scripts/benchmark_moe.py` compares dense vs. MOE models and prints the parameter and FLOP ratios.
See `docs/Implementation.md` and `docs/load_balance.md` for setup and measurement details.
