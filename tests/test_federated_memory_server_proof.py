...
class TestFederatedMemoryServerProof(unittest.TestCase):
    def test_sync_with_proof(self):
        ...
        s1 = FederatedMemoryServer(mem1, "localhost:50700", peers=["localhost:50701"], require_proof=True)
        s2 = FederatedMemoryServer(mem2, "localhost:50701", peers=["localhost:50700"], require_proof=True)
        s1.start(); s2.start()
    ...
    def test_bad_proof(self):
        ...
        with grpc.insecure_channel("localhost:50702") as ch:
            ...
            entry = memory_pb2.VectorEntry(id="a", vector=[0,0,0,0], metadata="a", timestamp=0, proof="bad")
